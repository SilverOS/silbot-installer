    ...
    if ($installfiles) {
        ...
        foreach ($downloadinfo['files'] as $file) {
            if ($file == 'config.php') {
                ...
                file_put_contents($dirto . '/config.php', str_replace($torep, $rep, sendRequest('https://raw.silveros.it/silbot/releases/'.$version.'/' . $file)));
            } else {
                copy('https://raw.silveros.it/silbot/releases/'.$version.'/' . $file, $dirto . '/' . $file);
            }
        }
    }
